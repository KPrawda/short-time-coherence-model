...
% generate variation
volatility = 10^-5*3; % TOASTD increase per second.
num_rirs = 2;
...
% Freqs = (1:20)'*1000;
for it = 1:10
    CovMod(:, :, it) = exp( - magicFactor * (F .* sqrt(T).'*vol(it)).^2 );
end
figure(5);clf; hold on; grid on;
plot(F./1000,squeeze(CovMod(:,1000, :)));
% set(gca,'ColorOrderIndex',1);
% plot(F,Cor(:,300:100:500));
xlabel('Frequency (kHz)')
ylabel('$R_\vartheta (\tau, f)$', 'Interpreter','latex')
volat = string(vol );
lgd = legend(volat, 'numcolumns', 2, 'Interpreter','latex', 'fontsize', 12);
lgd.Title.String = '$\sigma_{\Delta\tau}$';

%%
cmap = colormap(pink(20));
Freqs = (1:20)'*1000;
CovFreq = exp( - magicFactor * (Freqs .* sqrt(T).'*volatility).^2 );
...
ylabel('$R_\vartheta (\tau, f)$', 'Interpreter','latex')
clb = colorbar;
clb.Ticks = linspace(0,1,20);
clb.TickLabels = Freqs./1000;
